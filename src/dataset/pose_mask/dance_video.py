...
class HumanDanceVideoDataset(Dataset):
    ...
    def normalization(self, tensor):
        tensor[tensor>=0.001] = 1
        tensor[tensor<0.001] = 0 # normalize
        return tensor
    ...
    def __getitem__(self, index):
        ...
        ref_reader = VideoReader(mask_path.replace(original_id, 'c' + str(num).zfill(2)))
        ...
        for index in batch_index:
            ...
            bg_pil_mask_list.append(Image.fromarray(self.pil2binary_fg(Image.fromarray(img.asnumpy()))))
            img = bg_reader[index]
            bg_pil_image_list.append(Image.fromarray(img.asnumpy()))
        
        ref_img_idx = random.randint(0, video_length-2) ####
        ...
        pixel_masks_pose = self.augmentation(
            bg_pil_mask_list, self.cond_transform, state
        )
        pixel_values_pose = torch.concat((pixel_values_pose, self.normalization(pixel_masks_pose)),dim=0)
        ...
        pixel_values_ref_img=torch.concat((pixel_values_ref_img, self.normalization(pixel_values_ref_mask)),dim=0)

...
def pose_from_blender(filename):
    # 读取文本文件
    ...
    # 创建一个4x4的单位矩阵
    pose_matrices = np.eye(4)[None] * np.ones((len(frames), 1, 1))

    # 将位置和旋转信息填充到pose矩阵中
    pose_matrices[:, :3, :3] = rotation_matrices
    pose_matrices[:, :3, 3] = locations
...
if __name__ == "__main__":
    ...
    parser.add_argument("--character", type=str, const=True, default=False)
    args = parser.parse_args()

    setattr(args, "fragment_k", 1)
    os.makedirs(args.save_image_path, exist_ok=True)
    all_items=os.listdir(args.folder_smpl_path)
    filtered_items = [item for item in all_items if args.character in item]
    mesh_list = [os.path.join(args.folder_smpl_path, f) for f in filtered_items]

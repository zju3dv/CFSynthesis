...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_texture_path", type=str, required=True)
    parser.add_argument("--input_mask_path", type=str, required=True)
    parser.add_argument("--input_full_mask_path", type=str, required=True)
    parser.add_argument("--save_warp_image_path", type=str, required=True)
    args = parser.parse_args()
    
    img_m0 = cv2.imread(args.input_texture_path, cv2.IMREAD_GRAYSCALE)
    img_c0 = cv2.imread(args.input_mask_path)
    wrap_c0 = texture_padding(img_c0, img_m0)
    mask = cv2.imread(args.input_full_mask_path)
    wrap_c0[mask == 0] = 0
    cv2.imwrite(args.input_full_mask_path, wrap_c0)

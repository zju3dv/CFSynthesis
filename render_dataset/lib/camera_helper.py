...
def init_viewpoints(mode, sample_space, init_dist, init_elev, principle_directions, 
    use_principle=True, use_shapenet=False, use_objaverse=False, use_custom=False):
    ...
    if mode == "predefined":
        ...
        (
            ...
            azim_list, 
            sector_list
        ) = init_linear_viewpoints(sample_space, init_dist, init_elev)
...
def init_linear_viewpoints(sample_space, init_dist, init_elev):
    """
        y is up-axis
    """

    dist_list = [init_dist] * sample_space
    elev_list = [init_elev] * sample_space
    azim_list = (np.arange(sample_space)/sample_space*360).tolist()
    sector_list = ["good" for _ in elev_list] # HACK don't define sector names for now
...
# def init_camera(R, T, image_size, device):
def init_camera(dist, elev, azim, image_size, device):
    ...
    cameras = PerspectiveCameras(focal_length=(280,), R=R, T=T, device=device, image_size=image_size, principal_point=prp_screen, in_ndc=False,)

...
class Net(nn.Module):
    ...
    def forward(
        ...
        controlnet_latent_input = noisy_latents.squeeze(2)
        down_block_res_samples, mid_block_res_sample = self.controlnet(
            controlnet_latent_input,
            timesteps,
            encoder_hidden_states=clip_image_embeds, #.repeat(b, 1, 1),
        ...
        model_pred = self.denoising_unet(
            ...
            encoder_hidden_states=clip_image_embeds,
            down_block_additional_residuals=[ down_block_res_sample.unsqueeze(2) for down_block_res_sample in down_block_res_samples],
            mid_block_additional_residual=mid_block_res_sample.unsqueeze(2),
        ).sample
...
def main(cfg):
    ...
    reference_unet.requires_grad_(True)#(False)
    controlnet.requires_grad_(True) 
    pose_guider.requires_grad_(True) #(False)#
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(net):
                ...
                tgt_bg_img = batch["tgt_bg"].to(weight_dtype)
                with torch.no_grad():
                    latents = vae.encode(pixel_values).latent_dist.sample()
                    latents = latents.unsqueeze(2)  # (b, c, 1, h, w)
                    latents = latents * 0.18215
                    tgt_bg_img = vae.encode(tgt_bg_img).latent_dist.sample() * 0.18215
